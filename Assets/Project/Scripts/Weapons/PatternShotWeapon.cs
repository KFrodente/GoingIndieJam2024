...
public class PatternShotWeapon : Weapon
{
    protected override void Fire(Target target)
    {
        ...
        Instantiate(weaponData.projectile, transform.position, Quaternion.Euler(0, 0, angle), bc.transform).GetComponent<Projectile>().Initialize(target, (int)bc.GetStats().Damage);
